...
def determine_category(slug, title):
    ...
    if any(word in slug_lower or word in title_lower for word in ['florida', 'key west', 'keys']):
    ...
    elif any(word in slug_lower for word in ['north-carolina', 'virginia', 'maine', 'pennsylvania', 'new-york', 'massachusetts', 'new-jersey', 'maryland', 'assateague', 'bar-harbor', 'smoky-mountain', 'dollywood', 'blue-ridge', 'west-virginia', 'nuttallburg']):
        return 'East Coast'
    elif any(word in slug_lower for word in ['michigan', 'minnesota', 'kentucky', 'ohio', 'voyageurs', 'pictured-rocks', 'sleeping-bear']):
        return 'Mid-West'
    elif any(word in slug_lower for word in ['product', 'review', 'charging-station', 'solar', 'power', 'composting-toilet', 'air-conditioning']):
        ...
        return 'RV Life Tips'
    elif any(word in slug_lower or word in title_lower for word in ['roadtrip', 'road-trip', 'adventure', 'breakdown']):
...
blog_posts_dict = {}  # Use dict with slug as key to prevent duplicates
...
for html_file in html_files:
    ...
    try:
        ...
        # Clean up title
        if 'Full-time Living in a Class C Travel Tips' in title:
...
print(f"\nTotal: {len(blog_posts)} blog posts")
